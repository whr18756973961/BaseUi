package com.whr.lib.baseui.swipeback;
...
public class SwipeBackLayout extends FrameLayout {
	...
	private void init() {
		...
		setShadow(R.drawable.ic_shadow_left, EDGE_LEFT);
	...
	public void setEdgeOrientation(int orientation) {
		...
		if (orientation == EDGE_RIGHT || orientation == EDGE_ALL) {
			setShadow(R.drawable.ic_shadow_right, EDGE_RIGHT);
		}
	}
					...
					if (mFragment != null) {
						...
						if (fragmentList != null && fragmentList.size() > 1) {
							...
							for (int i = index - 1; i >= 0; i--) {
								...
								if (fragment != null && !fragment.isHidden() && fragment.getView() != null) {
				...
				if (mFragment != null) {
					...
					if (!mFragment.isDetached()) {
						...
						if (mFragment instanceof BaseFragment)
							((BaseFragment) mFragment).onHeaderBackPressed();
						else
