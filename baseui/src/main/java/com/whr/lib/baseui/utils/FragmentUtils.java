	...
	private static Fragment operateFragment(@NonNull FragmentManager fragmentManager,
		...
		switch (type) {
			case TYPE_ADD_FRAGMENT:
				if (NeedLoginUtils.needLogin(destFragment.getClass()) && !UiCoreHelper.getProxy().isLogin()) {
					name = UiCoreHelper.getProxy().loginFragment().getName();
					destFragment = Fragment.instantiate(UiCoreHelper.getProxy().getContext(), name);
				}
				if (srcFragment != null) ft.hide(srcFragment);
				//if (getTopShowFragment(fragmentManager) != null)
				ft.setCustomAnimations(R.anim.page_right_in, R.anim.page_stay, R.anim.page_stay, R.anim.page_right_out);
			...
			case TYPE_CHECK_ADD_FRAGMENT:
				...
				if (NeedLoginUtils.needLogin(destFragment.getClass()) && !UiCoreHelper.getProxy().isLogin()) {
					name = UiCoreHelper.getProxy().loginFragment().getName();
					destFragment = Fragment.instantiate(destFragment.getContext(), name);
				}
				//if (getTopShowFragment(fragmentManager) != null)
				ft.setCustomAnimations(R.anim.page_right_in, R.anim.page_stay, R.anim.page_stay, R.anim.page_right_out);
				ft.add(args.getInt(ARGS_ID), destFragment, name);
